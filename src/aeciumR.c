...
#define AUTHOR "Author: lyq19961011@gmail.com"
...
//******************---------------socket init----------------***************//
	...
	if (setsockopt (sockfd, SOL_SOCKET, SO_RCVTIMEO, (char *)&timeout,
				sizeof(timeout)) < 0)
		puts("[aeciumR]:Set sockopt Failed\n");
...
//******************---------------socket init----------------***************//
	...
	puts(VERSION);
	puts(AUTHOR);
	puts("[aeciumR]:Send Search Service Packet...Waiting For Response...");
	free(pkt);		
...
bool try_login(int sockfd, struct infoset * const pinfo){
	...
	*ppkt++ = 52;
	...
	if (login_status){
		...
		free(pkt_recv);
		puts("[aeciumR]:Login Success!");
		return login_status;
	}
	else{
		puts("[aeciumR]:Login Failed!Retry...");
		free(pkt_recv);
		return login_status;
	}
}
...
bool try_breathe(int sockfd, struct infoset * const pinfo ,long index){
	...
	*ppkt++ = 0x06;
	...
	bool login_status = (bool)pkt_recv[0x14];
	if (login_status){
		puts("[aeciumR]:Breathe Success");
		free(pkt_recv);
		return login_status;
	}
	else{
		puts("[aeciumR]:Breathe Failed!Retry...");
		free(pkt_recv);
		return login_status;
	}
}
